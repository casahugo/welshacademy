...
func TestAddFavoriteRecipe(t *testing.T) {
	os.Setenv("DATABASE_URL", "root@(db:3306)/welsh")
	application, _ := kernel.Boot()
	router := setupRouter(*application)
	...
	req, _ := http.NewRequest("POST", "/api/v1/favorite/2", nil)
	router.ServeHTTP(w, req)

	assert.Equal(t, 201, w.Code)
}

func TestRemoveFavoriteRecipe(t *testing.T) {
	os.Setenv("DATABASE_URL", "root@(db:3306)/welsh")
	application, _ := kernel.Boot()
	router := setupRouter(*application)
	...
	req, _ := http.NewRequest("DELETE", "/api/v1/favorite/1", nil)
	router.ServeHTTP(w, req)
